...
function renderProduct(){
	...
	for(i=0; i<product.length; i++) {
		result = product[i];
		html += '<tr data-id="' + result.id + '" id="sku-' + result.sku + '" data-image="" class="product-row tr-cat-' + result.category + ' tr-brand-'+result.brand+' tr-brand hidden font-normal">';
		...
		html2 += '<div data-id="' + result.id + '" class="product-row col-xs-12 col-sm-6 col-md-4 col-lg-4 margin-bottom-15 dv-cat-' + result.category + ' dv-brand-'+result.brand+' dv-brand hidden">';
		...
		html2 += ((result.isNew == 1) ? ' <img class="img-up" src="/images/icons/new.gif">' : '') + '</small>';
...
function loadProductImage(shop, sku){
	$.post($('#apiUrl').val()+'/product/image/list', {
		...
		sku: sku,
			...
			if (data.success) {
				$('#sku-'+sku).attr('data-image', data.result);
